...
const TypesSwitch = (state: any, action: DataProps) => {
  switch (action.type) {
    ...
    case 'loginByKey':
      return loginByKey(state, action);
    case 'loginGmail':
...
const loginByKey = (state: any, action: DataProps) => {
  console.log(action);
  if (action.estado != 'exito') return;
  state.usuarioLog = action.data;
  SStorage.setItem('usr_log', JSON.stringify(action.data));
...
const tarjetas = (state: any, action: DataProps) => {
  if (action.estado != 'exito') return;
  state.usuarioTarjetas = action.data;
  SStorage.setItem('tarjetas_user', JSON.stringify(action.data));
  // state.login = action.data;
};
