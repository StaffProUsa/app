...
class Registro extends React.Component {
  ...
  getregistro() {
    let data = {};
    if (this.key) {
      data = publicidad.Actions.getByKey(this.key, this.props);
      ...
      <SForm
        ...
        inputs={{
          foto_p: {
            type: 'image',
            isRequired: false,
            defaultValue: `${SSocket.api.root}publicidad/${this.key
        ...
        onSubmit={(values) => {
          if (this.key) {
            publicidad.Actions.editar({ ...data, ...values }, this.props);
          } else {
            publicidad.Actions.registro( values, this.props);
          }
        }}
      />
    );
  ...
  render() {
    var reducer = this.props.state[publicidad.component + 'Reducer'];
    if (reducer.type == 'registro' || reducer.type == 'editar') {
      if (reducer.estado == 'exito') {
        if (reducer.type == 'registro') this.key = reducer.lastRegister?.key;
        if (this.form) {
          this.form.uploadFiles(
            SSocket.api.root + 'upload/' + publicidad.component + '/' + this.key
          );
        }
        reducer.estado = '';
        SNavigation.goBack();
