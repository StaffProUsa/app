...
class Registro extends React.Component {
  ...
  getregistro() {
    let data = {};
    if (this.key) {
      data = banner.Actions.getByKey(this.key, this.props);
      ...
      <SForm
        ...
        inputs={{
          foto_p: {
            type: 'image',
            isRequired: false,
            defaultValue: `${SSocket.api.root}banner/${
              this.key
        ...
        onSubmit={(values) => {
          if (this.key) {
            banner.Actions.editar({...data, ...values}, this.props);
          } else {
            banner.Actions.registro(values, this.props);
          }
        }}
      />
    );
  ...
  render() {
    var reducer = this.props.state[banner.component + 'Reducer'];
    if (reducer.type == 'registro' || reducer.type == 'editar') {
      if (reducer.estado == 'exito') {
        if (reducer.type == 'registro') this.key = reducer.lastRegister?.key;
        if (this.form) {
          this.form.uploadFiles(
            SSocket.api.root + 'upload/' + banner.component + '/' + this.key
          );
        }
        reducer.estado = '';
        SNavigation.goBack();
