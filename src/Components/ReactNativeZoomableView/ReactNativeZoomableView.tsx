...
class ReactNativeZoomableView extends Component<ReactNativeZoomableViewProps, ReactNativeZoomableViewState> {
  ...
  mousewheel = (evt) => {
    ...
    this.state.zoomLevel = this.state.zoomLevel + (evt.deltaY * -0.001);
    // this.zoomBy(this.state.zoomLevel + evt.deltaY);
  ...
  _getBoxDimensions = (layoutEvent) => {
    ...
    this.setState({
      originalWidth: this.props.originalWidth ?? width,
      originalHeight: this.props.originalHeight ?? height,
    });
  };
