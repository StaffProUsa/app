...
class Confirmar extends React.Component {
  ...
  componentDidMount() {
    this.setState({ usuario: Model.usuario.Action.getUsuarioLog() })
  }
  ...
  getFormTarjeta() {
    ...
    const permisoVenta = Model.usuarioPage.Action.getPermiso({
      url: "admin/venta",
      permiso: "vender",
      loading: ""
    })
      ...
      <>
        <SView col={'xs-12 '} center>
          <SView col={'xs-11'} >
            ...
            <SHr />
            {!permisoVenta ? null :
              <SText fontSize={12} underLine font={'Roboto'} color={STheme.color.warning} onPress={() => {
                SNavigation.navigate("/usuario", {
                  onSelect: (e) => {
                    this.setState({ usuario: null })
                    new SThread(200, "Asdasd").start(() => {
            ...
            <SForm
              ...
              inputs={{
                razon_social: {
                  icon: <SView width={60} height={50}>
                    <SImage src={Model.usuario._get_image_download_path(SSocket.api, this.state?.usuario?.key)} enablePreview />
                  </SView>,
                ...
                correo: {
                  icon: <SView width={60} height={50} center>
                    <SText>{"@"}</SText>
                  </SView>,
              ...
              onSubmit={(values) => {
                ...
                values.key_usuario = usuario.key;
