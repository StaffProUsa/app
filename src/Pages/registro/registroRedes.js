...
import { SForm, SHr, SNavigation, SPage, SPopup, SText, STheme, SView, SIcon, SLanguage, SThread, SLoad } from 'servisofts-component';
...
import MDL from '../../MDL';
class registroRedes extends Component {
    constructor(props) {
        super(props);
        this.state = {
            envio: 0
    ...
    componentDidMount() {
        ...
        // if (dataUser) {
        //     this.setState({datoUser: dataUser})
    ...
    //     return 
    render() {
        if (!this.load_data()) return MDL.validaciones.componentDidMount();
        // var data = this.state.datoUser;
                            ...
                            inputs={{
                                Nombres: {
                                    label: SLanguage.select({
                                        es: "Nombre",
                                        en: "Name"
                                    }), placeholder: SLanguage.select({ es: "Nombre", en: "First Name" }), isRequired: true, defaultValue: this.data?.Nombres, icon: this.icon("InputUser")
                                },
                                Apellidos: { label: SLanguage.select({ es: "Apellidos", en: "Last name" }), isRequired: true, placeholder: apellidos, defaultValue: this.data?.Apellidos, icon: this.icon("InputUser") },
                                fecha_nacimiento: { label: SLanguage.select({ es: "Fecha de nacimiento", en: "Date of Birth" }), placeholder: fecha, isRequired: false, defaultValue: this.data?.fecha_nacimiento, type: "date", icon: this.icon("cumple") },
                                estado_civil: {
                                ...
                                // },
                                Telefono: {
                                    label: SLanguage.select({ es: "Número de teléfono", en: "Phone Number" }), isRequired: true, placeholder: SLanguage.select({ es: "Número de teléfono", en: "Phone Number" }), type: "phone", defaultValue: this.data?.Telefono,
                                },

                                Correo: { label: SLanguage.select({ es: "Correo", en: "Email" }), isRequired: true, placeholder: SLanguage.select({ es: "Correo", en: "Email" }), type: "email", defaultValue: this.data?.Correo, icon: this.icon("InputEmail") },
                                direccion: {
                                    label: SLanguage.select({ es: "Dirección de domicilio", en: "Home Address" }), isRequired: true, placeholder: SLanguage.select({ es: "Dirección de domicilio", en: "Home Address" }), defaultValue: this.data?.direccion, icon: this.icon("direccion")
                                },
                                nivel_ingles: {
...
export default connect(initStates)(registroRedes);