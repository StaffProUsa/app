...
export default class ClockAll extends Component {
    constructor(props) {
        super(props);
        this.state = {
        };
    ...
    render() {
        //ARRAY DE STAFF_USUARIO.KEY
        console.log(this.props.data)
        const staff_usuarios = this.props.data;
        const staff_all = this.props.data;
        return <SView width={300} height={300} backgroundColor={STheme.color.background} style={{
            borderRadius: 16,
            alignItems: "center"
        }} withoutFeedback padding={8}>
            <SHr h={16} />
            <SText bold fontSize={20}>{"All Clock In / Out"}</SText>
            <SHr h={16} />
            <SView col={"xs-12"} row center>
                <SText bold >{"Clock IN"}</SText>
                <SView flex />
                {staff_usuarios?.fecha_ingreso ? this.btnDel({
                    onPress: () => {
                        staff_usuarios.fecha_ingreso = null
                        this.setState({ ...this.state })
                    }
                }) : this.btnAdd({
                    onPress: () => {
                        staff_usuarios.fecha_ingreso = new SDate().toString("yyyy-MM-ddThh:mm:ssTZD")
                        this.setState({ ...this.state })
                    }
                })}
            </SView>
            <SHr h={8} />
            {this.renderFechaHora({ fecha: staff_usuarios?.fecha_ingreso, key: "clockin" })}
            <SHr h={16} />
            <SView col={"xs-12"} row center>
                <SText bold >{"Clock OUT"}</SText>
                <SView flex />
                {staff_usuarios?.fecha_salida ? this.btnDel({
                    onPress: () => {
                        staff_usuarios.fecha_salida = null
                        this.setState({ ...this.state })
                    }
                }) : this.btnAdd({
                    onPress: () => {
                        staff_usuarios.fecha_salida = new SDate().toString("yyyy-MM-ddThh:mm:ssTZD")
                        this.setState({ ...this.state })
                    }
                })}
            </SView>
            <SHr h={8} />
            {this.renderFechaHora({ fecha: staff_usuarios?.fecha_salida, key: "clockout" })}
            <SView flex />
            <Btn width={100} onPress={() => {
                ...
                delete staff_all.fecha_ingreso;
                delete staff_all.fecha_salida;

                SSocket.sendPromise({
                    component: "staff_usuario",
                    type: "asignarClockAll",
                    key_usuario: Model.usuario.Action.getKey(),
                    key_staff_usuarios: staff_all,
                    fecha_ingreso: full_in,
                    fecha_salida: full_out
                }).then(e => {
                    this.props.onChange()
