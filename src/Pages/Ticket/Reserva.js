...
class Reserva extends React.Component {
  ...
  paint_marcar(mesa) {
    if (!this?._ref_canvas?.ctx) return;
    const ctx = this?._ref_canvas?.ctx;
    ctx.strokeStyle = "#f00"
  ...
  paint_desmarcar(mesa) {
    ...
    // ctx.lineWidth = 2;
    // ctx.strokeRect(isla.x, isla.y, isla.w, isla.h)
    ctx.clearRect(mesa.x, mesa.y, mesa.w, mesa.h)
  }
  getCanvas() {
    ...
    Object.values(this.state.marcadas).map(a => {
      if (!marcadas[a.key]) {
        this.paint_desmarcar(a);
      }
    })
    ...
    const permiso_reasignar = Model.usuarioPage.Action.getPermiso({ url: "/ticket/reservaevento", permiso: "reasignar_mesa", });
    ...
    return <CanvasPlanimetria
      ...
      paint={(ref) => {
        ...
        console.log("carrito_data", carrito_data);
        carrito_data.filter(o => o.key_evento == this.key).map((ca) => {
          if(!ca.mesas) return;
          ca?.mesas.map((mesa) => {
            this.paint_marcar(mesa);
